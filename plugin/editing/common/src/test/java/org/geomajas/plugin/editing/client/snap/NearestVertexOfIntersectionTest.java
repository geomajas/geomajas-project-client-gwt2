...
/**
 * Test cases for the {@link NearestVertexOfIntersection}.
 * 
 * @author Pieter De Graef
 */
public class NearestVertexOfIntersectionTest {

	private static final double DELTA = 0.0001;

	private final NearestVertexOfIntersection algorithm = new NearestVertexOfIntersection();
	...
	public void testLinearRing() {
		...
		result = algorithm.snap(new Coordinate(2, 1), 3);
		Assert.assertEquals(0.0, result.getX(), DELTA);
		Assert.assertEquals(0.0, result.getY(), DELTA);

		result = algorithm.snap(new Coordinate(5, 4.5), 3);
		Assert.assertEquals(4.0, result.getX(), DELTA);
		Assert.assertEquals(4.0, result.getY(), DELTA);

		result = algorithm.snap(new Coordinate(1, 11), 10);
		Assert.assertEquals(1.0, result.getX(), DELTA);
	...
	public void testPolygon() {
		...
		result = algorithm.snap(new Coordinate(2, 1), 100);
		Assert.assertEquals(0.0, result.getX(), DELTA);
		Assert.assertEquals(0.0, result.getY(), DELTA);

		result = algorithm.snap(new Coordinate(5, 4.5), 100);
		Assert.assertEquals(5.0, result.getX(), DELTA);
		Assert.assertEquals(4.5, result.getY(), DELTA);

		result = algorithm.snap(new Coordinate(1, 11), 100);
		Assert.assertEquals(1.0, result.getX(), DELTA);
		Assert.assertEquals(11.0, result.getY(), DELTA);
	}
}
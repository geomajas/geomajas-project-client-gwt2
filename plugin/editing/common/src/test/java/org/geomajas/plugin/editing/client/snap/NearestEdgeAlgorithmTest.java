 ...
 */
public class NearestEdgeAlgorithmTest {

	private static final double DELTA = 0.0001;

	private NearestEdgeSnapAlgorithm algorithm = new NearestEdgeSnapAlgorithm();
	...
	public void testLineString() {
		...
		Coordinate result = algorithm.snap(new Coordinate(1, 1), 0.9);
		Assert.assertEquals(1.0, result.getX(), DELTA);
		Assert.assertEquals(1.0, result.getY(), DELTA);
		...
		result = algorithm.snap(new Coordinate(9, 9), 2);
		Assert.assertEquals(9.0, result.getX(), DELTA);
		Assert.assertEquals(10.0, result.getY(), DELTA);
		...
		result = algorithm.snap(new Coordinate(8, 11), 2);
		Assert.assertEquals(8.0, result.getX(), DELTA);
		Assert.assertEquals(10.0, result.getY(), DELTA);
	}
}
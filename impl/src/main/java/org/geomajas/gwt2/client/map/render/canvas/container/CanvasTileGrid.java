...
public class CanvasTileGrid implements IsWidget {
	...
	public void addBounds(Bbox bounds) {
		...
		if (!range.containsRange(newRange)) {
			...
			int newWidth = union.getNx() * tileConfiguration.getTileWidth();
			int newHeight = union.getNx() * tileConfiguration.getTileHeight();
			if(!CanvasSizeTest.getInstance().isTooBig(newWidth, newHeight)) {
				// canvas too big, shrink to new bounds
				union = newRange;
			}

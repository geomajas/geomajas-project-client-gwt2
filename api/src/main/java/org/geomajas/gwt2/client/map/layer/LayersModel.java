...
/**
 * <p> Definition of the model that describes the layers that make up a map. The methods within this model all revolve
 * around layer manipulation. This means that there are methods here to add and remove layers, to move layers up and
 * down, to retrieve layers by their identifiers or index in the list, etc... </p>
 *
...
public interface LayersModel {
	 ...
	 *
	 * @param layer The layer to place at a new position.
	 * @param index The new layer order position in the layer array:
	 ...
	 * any time, layers rendered through HTML will always lie behind layers rendered through SVG/VML objects.<br/> Note
	 * also that vector layers can be rendered as images, in which case they will count as HTML layers here.
	 *
	 * @param layer The layer to move more to the front.
	 * @return Returns if the re-ordering was successful or not.
	 */
